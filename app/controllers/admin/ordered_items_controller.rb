class Admin::OrderedItemsController < ApplicationController
  before_action :authenticate_admin!
	...
	def update
		...
		# 紐づく商品の制作ステータスがひとつでも「制作中」になったら
		if @ordered_item.making_status == 2
		    # 注文ステータスを「製作中」に"自動更新"
		...
		elsif @order.ordered_items.count ==  @order.ordered_items.where(making_status: 3).count
			@order.update(status: 3) #注文ステータスを「発送準備中」に"自動更新"
		end
